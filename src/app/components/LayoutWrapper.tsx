...
const getLayoutType = (pathname: string): LayoutType => {
  ...
  const customPages = [''];
  const customPatterns = ['/notes/']; // For dynamic routes like /notes/[slug] (individual articles)
  
  return customPages.includes(pathname) || 
         customPatterns.some(pattern => pathname.startsWith(pattern)) 
         ? 'custom' : 'basic';
};
